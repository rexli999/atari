...
def learn(env,
    ...
    # former_vars=tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='target_q_func')
    # set up placeholders
    ...
    #    if 'Conv' in var.name:
    #        transfer_update.append(var_target.assign(var))
    # transfer_update = tf.group(*update_target_fn)
    ...
    transfer_update = []
    # update_target_fn = []
    for var, var_target in zip(sorted(target_q_func_vars_tf,        key=lambda v: v.name),
                               sorted(q_func_vars_tf, key=lambda v: v.name)):
        transfer_update.append(var_target.assign(var))
    transfer_update = tf.group(*transfer_update)
    ...
    # # q_new_func_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='tf_q_func/action_value')
    # to_initialize = []
    # to_initialize.extend(q_func_vars)
    # to_initialize.extend(target_q_func_vars)
    # for i in all_vars:
    #     if 'beta' in i.name or 'Adam' in i.name:
    #         to_initialize.append(i)
    ...
    for t in itertools.count():
        ...
        if (t > learning_starts and
            ...
            if not model_initialized:
                #TODO: initialize the variable in the fc layer only
                # for i in to_initialize:
                #     print(i.name)
                initialize_interdependent_variables(session, tf.global_variables(), {
                   obs_t_ph: obs_t_batch,
                   obs_tp1_ph: obs_tp1_batch,
            ...
            if not if_model_loaded:
                ...
                to_assign = []
                for j in target_q_func_vars_tf:
                    to_assign.append(j.assign(weights[j.name]))
                session.run(tf.group(*to_assign))
                session.run(transfer_update)
